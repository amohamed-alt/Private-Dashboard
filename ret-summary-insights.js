(() => {
  ...
  function fallbackSummary() {
    ...
    const topRows = RET_TEAM.map((name) => ({
      ...
      ytdProspective: { totalCashing: null, percent: null },
    }));
    const topTotal = {
      name: 'Total',
      retTarget: topRows.reduce((sum, row) => sum + num(row.retTarget), 0),
      bookedRet: null,
      cashedRet: null,
      receivables: null,
      achievement: { bookingRet: null, cashingRet: null },
    ...
    const projectionRows = RET_TEAM.map((name) => {
      ...
      return { name, lost: num(stat.lost), pendingHigh: num(stat.pendingHigh), pendingLow: num(stat.pendingLow), pendingMedium: num(stat.pendingMedium), late: 0, totalProjection: { worst, medium, best, outstanding: best } };
    });
    const projectionTotal = projectionRows.reduce((total, row) => ({
      name: 'Total', isTotal: true,
      lost: add(total.lost, row.lost), pendingHigh: add(total.pendingHigh, row.pendingHigh), pendingLow: add(total.pendingLow, row.pendingLow), pendingMedium: add(total.pendingMedium, row.pendingMedium), late: add(total.late, row.late),
      totalProjection: { worst: add(total.totalProjection?.worst, row.totalProjection.worst), medium: add(total.totalProjection?.medium, row.totalProjection.medium), best: add(total.totalProjection?.best, row.totalProjection.best), outstanding: add(total.totalProjection?.outstanding, row.totalProjection.outstanding) },
    }), { totalProjection: {} });
    return { topRows, topTotal, projectionRows, projectionTotal, closingYear: { reps: [], total: null, delta: [] }, byName: {}, fallback: true };
  }

  function summary() {
    const exact = DATA.summaryRet;
    if (exact && (exact.topRows?.length || exact.projectionRows?.length)) return { ...exact, fallback: false };
