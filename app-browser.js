...
const statusPillTone = (s='') => {
  ...
  if (key.includes('done')) return 'green';
  if (key.includes('postpon')) return 'amber';
  if (key.includes('cancel')) return 'red';
  if (key.includes('to do')) return 'blue';
  if (key.includes('n/a') || key.includes('na')) return 'gray';
  return 'gray';
};
...
function scheduleFor(item, month='All') {
  const schedule = Array.isArray(item?.renewalSchedule) ? item.renewalSchedule : [];
  if (month === 'All') return schedule;
  return schedule.filter((event) => event.month === month);
}

function scheduleValue(item, month='All') {
  const scoped = scheduleFor(item, month);
  if (month === 'All') return scoped.length ? scoped.reduce((sum, event) => sum + number(event.amount), 0) : number(item.updatedValue);
  return scoped.reduce((sum, event) => sum + number(event.amount), 0);
}

function renewalMonthText(item, month='All') {
  const scoped = scheduleFor(item, month);
  if (!scoped.length) return '—';
  return scoped.map((event) => event.month).join(' · ');
}

function renewalValueText(item, month='All') {
  const scoped = scheduleFor(item, month);
  if (!scoped.length) return '—';
  return scoped.map((event) => money(event.amount)).join(' · ');
}

function RenewalMonthCell({ item, month }) {
  const scoped = scheduleFor(item, month);
  if (!scoped.length) return html`<span className="empty-cell">—</span>`;
  return html`<div className="renewal-month-list">${scoped.map((event, index) => html`<span key=${`${event.month}-${index}`}>${event.month}</span>`)}</div>`;
}

function RenewalValueCell({ item, month }) {
  const scoped = scheduleFor(item, month);
  if (!scoped.length) return html`<span className="empty-cell">—</span>`;
  return html`<div className="renewal-value-list">${scoped.map((event, index) => html`<span key=${`${event.month}-${index}`}>${money(event.amount)}</span>`)}</div>`;
}
...
function StatusMix({ accounts, month='All' }) {
  const statuses=['Renewed','Pending High','Pending Medium','Pending Low','Lost'];
  const totals=Object.fromEntries(statuses.map(s=>[s,0]));
  accounts.forEach(a=>{ if (totals[a.renewalStatus] !== undefined) totals[a.renewalStatus]+=scheduleValue(a, month); });
...
function AccountsTable({ accounts, month='All', limit=12 }) {
  const [all,setAll]=useState(false); const rows=all?accounts:accounts.slice(0,limit);
  return html`<div><div className="table-wrap"><table><thead><tr><th>Client</th><th>RM</th><th>CSM</th><th>Product</th><th>Status</th><th className="renewal-month-cell">Renewal Month</th><th className="renewal-value-cell">Renewal Value</th><th className="num">Updated Value</th></tr></thead><tbody>${rows.map(a=>html`<tr key=${a.id}><td><strong>${a.clientName||'—'}</strong><small>${a.location||'—'}</small></td><td>${a.rm||'—'}</td><td>${a.csm||'—'}</td><td>${a.product||'—'}</td><td><span className=${`pill ${statusTone(a.renewalStatus)}`}>${a.renewalStatus||'—'}</span></td><td className="renewal-month-cell"><${RenewalMonthCell} item=${a} month=${month}/></td><td className="renewal-value-cell"><${RenewalValueCell} item=${a} month=${month}/></td><td className="num">${money(a.updatedValue)}</td></tr>`)}</tbody></table></div>${accounts.length>limit?html`<button className="more" onClick=${()=>setAll(!all)}>${all?'Show less':`See all ${accounts.length}`}</button>`:null}</div>`;
}

function ActionsTable({ actions, month='All', limit=12 }) {
  const [all,setAll]=useState(false); const rows=all?actions:actions.slice(0,limit);
  return html`<div><div className="table-wrap"><table className="actions-table"><thead><tr><th>Client</th><th>Action</th><th>Owner</th><th>RM / CSM</th><th>Due</th><th>Action Status</th><th className="renewal-month-cell">Renewal Month</th><th className="renewal-value-cell">Renewal Value</th></tr></thead><tbody>${rows.map(a=>{const timing=actionTiming(a); return html`<tr key=${a.id}><td><strong>${a.clientName||'—'}</strong><small>${money(a.updatedValue)}</small></td><td>${a.action||'—'}${a.notes?html`<small>${a.notes}</small>`:null}</td><td>${a.owner||'—'}</td><td>${a.rm||'—'} / ${a.csm||'—'}</td><td>${a.dueDateRaw||'—'}<span className=${`timing ${timing.toLowerCase().replaceAll(' ','-')}`}>${timing}</span></td><td>${a.actionStatus?html`<span className=${`pill ${statusPillTone(a.actionStatus)}`}>${a.actionStatus}</span>`:'—'}</td><td className="renewal-month-cell"><${RenewalMonthCell} item=${a} month=${month}/></td><td className="renewal-value-cell"><${RenewalValueCell} item=${a} month=${month}/></td></tr>`;})}</tbody></table></div>${actions.length>limit?html`<button className="more" onClick=${()=>setAll(!all)}>${all?'Show less':`See all ${actions.length}`}</button>`:null}</div>`;
}
...
function App() {
  ...
  const filteredRenewalValue = accounts.reduce((s,a)=>s+scheduleValue(a, activeMonth),0);
  const total=accounts.reduce((s,a)=>s+Number(a.updatedValue||0),0);
  const renewed=accounts.filter(a=>a.renewalStatus==='Renewed').reduce((s,a)=>s+scheduleValue(a, activeMonth),0);
  const high=accounts.filter(a=>a.renewalStatus==='Pending High').reduce((s,a)=>s+scheduleValue(a, activeMonth),0);
  const current=MONTHS[new Date().getMonth()];
  const thisMonth=accounts.reduce((s,a)=>s+scheduleValue(a,current),0);
  ...
  const totalSub = activeMonth === 'All' ? 'All filtered renewal values' : `${activeMonth} renewal value`;
  const kpis=html`<div className="kpi-grid"><${Kpi} label="Accounts" value=${accounts.length.toLocaleString()} sub="Current scope" icon="□"/><${Kpi} label="Filtered Renewal Value" value=${money(filteredRenewalValue)} sub=${totalSub} tone="highlight" icon="$"/><${Kpi} label="Renewed" value=${money(renewed)} sub=${activeMonth==='All'?'Updated value':`${activeMonth} value`} tone="green" icon="✓"/><${Kpi} label="Pending High" value=${money(high)} sub=${activeMonth==='All'?'Updated value':`${activeMonth} value`} tone="blue" icon="↗"/><${Kpi} label=${`Renewals in ${current}`} value=${money(thisMonth)} sub="Monthly columns" tone="amber" icon="▦"/></div>`;
  ...
  ${entity?html`<div className="stack">${kpis}${entity.type==='rm'?html`<${Panel} title="Management Forecast"><${Forecast} management=${management} scenario=${scenario} setScenario=${setScenario}/></${Panel}>`:null}<div className="two"><${Panel} title="Status Mix" subtitle=${activeMonth==='All'?'All filtered renewal values':`${activeMonth} renewal value`}><${StatusMix} accounts=${accounts} month=${activeMonth}/></${Panel}><${Panel} title="Renewal Timeline"><${Timeline} accounts=${accounts}/></${Panel}></div><${Panel} title="Accounts"><${AccountsTable} accounts=${[...accounts].sort((a,b)=>b.updatedValue-a.updatedValue)} month=${activeMonth}/></${Panel}><${Panel} title="Actions"><${ActionsTable} actions=${actions} month=${activeMonth}/></${Panel}></div>`:null}
  ...
  ${page==='accounts'?html`<div className="stack">${kpis}<${Panel} title="Accounts Explorer" subtitle=${`${accounts.length} matching accounts · ${money(filteredRenewalValue)} renewal value`}><${AccountsTable} accounts=${[...accounts].sort((a,b)=>b.updatedValue-a.updatedValue)} month=${activeMonth} limit=${18}/></${Panel}></div>`:null}
  ${page==='actions'?html`<div className="stack"><div className="kpi-grid four"><${Kpi} label="Actions" value=${actions.length} sub="Rows with Action" icon="✓"/><${Kpi} label="Action Renewal Value" value=${money(actions.reduce((sum, action)=>sum+scheduleValue(action, activeMonth),0))} sub=${activeMonth==='All'?'Filtered actions':'Filtered action month'} tone="highlight" icon="$"/><${Kpi} label="Due This Week" value=${actions.filter(a=>actionTiming(a)==='Due This Week').length} tone="amber" icon="▦"/><${Kpi} label="No Due Date" value=${actions.filter(a=>actionTiming(a)==='No Due Date').length} tone="purple" icon="—"/></div><section className="filters action-filters"><${Select} label="Owner" value=${actionFilters.owner} options=${DATA.filters?.actionOwners||[]} onChange=${v=>setActionFilters(a=>({...a,owner:v}))}/><${Select} label="Action Status" value=${actionFilters.status} options=${DATA.filters?.actionStatuses||[]} onChange=${v=>setActionFilters(a=>({...a,status:v}))}/><${Select} label="Due Timing" value=${actionFilters.timing} options=${['Overdue','Due Today','Due This Week','Upcoming','No Due Date','Completed']} onChange=${v=>setActionFilters(a=>({...a,timing:v}))}/></section><${Panel} title="Action Center" subtitle=${`${money(actions.reduce((sum, action)=>sum+scheduleValue(action, activeMonth),0))} renewal value in current filters`}><${ActionsTable} actions=${actions} month=${activeMonth}/></${Panel}></div>`:null}
